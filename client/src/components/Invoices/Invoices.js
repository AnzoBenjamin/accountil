...
const Invoices = () => {
  ...
  const [rowsPerPage, setRowsPerPage] = React.useState(10); // Set default rowsPerPage
  ...
  useEffect(() => {
    ...
    if (userId) {
      dispatch(getInvoicesByUser({ search: userId }));
    }
  }, [dispatch]);

  useEffect(() => {
      console.log('Invoices in component:', rows);
  }, [rows]);
  ...
  if (!getUserId()) {
      history.push('/login');
      return null; // Prevent rendering below if redirecting
  }
